...
class Modul_model extends CI_Model
{
    ...
    public function JumlahUser($no_kelompok = NULL)
    {
    ...
    {
        $query = "SELECT `anggota_kelompok`.`id` , `anggota_kelompok`.`nrp` as `nrp` , `user`.`name` as 'name' , `anggota_kelompok`.`no_kelompok` FROM `anggota_kelompok` 
    ...
    {
        $id = $this->input->post('id', true);
        $query = "SELECT `jadwal`.`id`, `user`.`name` as 'name',`user`.`nrp`, `modul`.`modul` as 'modul_id', `kelompok_asisten`.`nrp` as 'kelompok_asisten'
                    `modul`.`name` as 'modul', `jadwal`.`jadwal` FROM `user` 
                    INNER JOIN `jadwal` ON `user`.`nrp` = `jadwal`.`nrp` 
                    INNER JOIN `modul` ON `modul`.`modul` = `jadwal`.`modul_id` 
                    LEFT JOIN `kelompok_asisten` ON `user`.`nrp` = `kelompok_asisten`.`nrp`
                    WHERE `jadwal`.`id`='$id'";
